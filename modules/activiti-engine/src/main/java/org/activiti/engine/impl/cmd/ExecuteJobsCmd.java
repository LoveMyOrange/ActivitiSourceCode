...
public class ExecuteJobsCmd implements Command<Object>, Serializable {
  ...
  public Object execute(CommandContext commandContext) {
    ...
    if (jobId == null && job == null) {// 对于一个不存在的作业来说  执行下面的代码就没有意义了
    ...
    if (job == null) {
    	job = commandContext
    	  .getJobEntityManager()
    		.findJobById(jobId); //  从act_ru_job表中查询数据
    }
    
    if (job == null) { //如果没哟查询到数据 博爱错
    ...
    JobExecutorContext jobExecutorContext = Context.getJobExecutorContext(); //获取作业处理上下文
    if (jobExecutorContext != null) { // if null, then we are not called by the job executor     
      jobExecutorContext.setCurrentJob(job); //将job设置到 jobExecutorContext对象中
    }
    // 该类可以在作业失败之后调用 JObRetryCmd 再次重试执行作业, 引擎默认重试3次
    ...
    try {
      ...
      job.execute(commandContext);//处理作业
