...
public abstract class JobExecutor {
  ...
  public void start() {
    ...
    ensureInitialization();    //确保必要的类已经被实例化
    startExecutingJobs();//启动作业执行器
    isActive = true;//设置为true
  }
  ...
  * */
  ...
  protected void ensureInitialization() { 
  	if (acquireJobsCmd == null) {
  		acquireJobsCmd = new AcquireJobsCmd(this);//this 为DefaultJobExecutor类 (前提 开发人员用的是默认的作业执行器)
  	}
  	if (acquireJobsRunnable == null) {
  	  /*
  	  AcquireJobsRunnableImpl 该类实现了 AcquireJobsRunnable 继承了 Runnabe接口
  	  因此该类是非常重要的一个 线程类 , 负责定时查询并且执行作业
  	   */
