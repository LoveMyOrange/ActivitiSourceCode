 ...
 * @author Joram Barrez
 *
 * 对 EventLoggerEventHandler 中的 () 进行实现 , 作为 所有事件 处理类的模板类 存在
 * 全局统筹 EventLogEntryEntity 实体对象的 生成 工作
 */
public abstract class AbstractDatabaseEventLoggerEventHandler implements EventLoggerEventHandler {
	...
	protected EventLogEntryEntity createEventLogEntry(String type,
		...
		try {
			eventLogEntry.setData(objectMapper.writeValueAsBytes(data)); //该() 将data 转化为byte
		} catch (Exception e) {
