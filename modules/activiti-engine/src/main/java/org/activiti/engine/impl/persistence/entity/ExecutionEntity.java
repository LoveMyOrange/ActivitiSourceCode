...
public class ExecutionEntity extends VariableScopeImpl implements ActivityExecution, ExecutionListenerExecution, Execution, PvmExecution, 
  ...
  * */
  public void start() {
    /*
    判断startingExecution
    如果该值为null 并且  isProcessInstanceType  true
    则重新实例化 StartingExecution
    主要是为了确保当前执行的是 流程实例 而非执行实例
    为何这样操作??
  ...
  public void take(PvmTransition transition, boolean fireActivityCompletionEvent) {
    //如果 fireActivityCompletionEvent true 调用  giao()  主要用于转发 ACTIVITYI_COMPLETED
  	if (fireActivityCompletionEvent) {
	  	fireActivityCompletedEvent();
  	}
  	//校验ExecutionEntity对象中的 transition属性值, 如果该属性值 不为空 , 直接报错  transitin属性用来存储已经执行过的连线
  ...
  /*
  *对于该()的操作
  * 参数为 executionOperation  接口中定义的 原子类 StartAtomicOperationProcessStart
  * 看到这里的处理难免有点疑惑
  * 因为 的开始节点是流程启动的源头
  * 则应该以开始节点的原子类作为入口 ,
  * 为何程序首先执行 StartAtomicOperationProcessStart类呢??
  *   为何??
  *     因为开发人员可以在process中定义监听器,
  * 如果在process元素和startEvent中分别定义了一个事件类型为start的监听器, 那么程序首先执行哪一个监听器呢???
  * 毋庸置疑,当前是首先执行process元素中的定义的事件类型为start的监听器 ,
  * 因此程序会首先执行 AtomicOperationProcessStart 原子类
  * */
  public void performOperation(AtomicOperation executionOperation) {
    //判断是否需要异步处理 ,开发人员可以在流程文档中 为节点定义是否需要异步处理属性  比如 activiti:async=true
  ...
  * */
  protected void performOperationSync(AtomicOperation executionOperation) {
    /*
    直接将 流程实例运转的工作 委托给Context 的 performOperation 进行处理
...
/*
*   异步节点处理
*     如果 executionOperation.isAsync 为true 紫萼调用 此() 进行处理
* */
  protected void scheduleAtomicOperationAsync(AtomicOperation executionOperation) {
    //1)实例化MessageEntity类 该类的父类是JobEntity (ACT_RU_JOB 表的实体类 )
    MessageEntity message = new MessageEntity();
    //填充属性值  executionId ,processInstanceId, processDefinitionId
    ...
    //配置 getLockTimeAsyncJobWaitTime 属性值 ,默认为60
  ...
  * */
  ...
  public List<JobEntity> getJobs() {
    ...
    /*
    * 调用 getJobsInternal() 获取 List<JobEntity> 集合
    * 然后将 jobEntity 添加到 该集合中
    * */
