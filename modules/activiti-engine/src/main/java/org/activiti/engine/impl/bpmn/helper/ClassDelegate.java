 ...
 * 对接口 TaskListener  和ExecutionListener 中的() 进行实现
 * 负责调度执行class方式创建 的监听器 , 其内部持有需要调用的执行监听器实例,
 * 任务监听器实例, 并提供了 实例化监听器的() 以及对实例对象进行属性填充的()
 *
 *
 */
public class ClassDelegate extends AbstractBpmnActivityBehavior implements TaskListener, ExecutionListener, SubProcessActivityBehavior {
  ...
  * */
  ...
  public void notify(DelegateExecution execution) throws Exception {
    if (executionListenerInstance == null) { //非空校验
      executionListenerInstance = getExecutionListenerInstance();//实例化需要执行的监听器
    }
    /*
    调用 executionlistenerinvocation对象的invoke()   invoke() 最终会触发执行监听器对象中的notify()
...
* */
  protected ExecutionListener getExecutionListenerInstance() {
    //根据类名和 属性 两个值 实例化 执行监听器
    Object delegateInstance = instantiateDelegate(className, fieldDeclarations);
    //如果执行监听器没有实现 ExecutionListener 或者JavaDelegate 接口 则直接报错 .
//    在这里进行类型转化主要是为了方便对具体监听器进行全局统一调度 并约束执行监听器的实现类必须实现以上两个接口中的任意一个
    if (delegateInstance instanceof ExecutionListener) { //类型转化
  ...
  * */
  public static Object defaultInstantiateDelegate(String className, List<FieldDeclaration> fieldDeclarations) {
    Object object = ReflectUtil.instantiate(className);//通过className 反射
    applyFieldDeclaration(fieldDeclarations, object);//对object对象进行属性填充
    return object;
  }
  ...
  * */
  ...
  /*
  将执行监听器中的表达式属性信息值 填充到监听器实例对象中,
  因为 执行监听器中 可以配置 多个 表达式  属性
  * */
  public static void applyFieldDeclaration(List<FieldDeclaration> fieldDeclarations, Object target, boolean throwExceptionOnMissingField) {
    if(fieldDeclarations != null) {//非空判断
  ...
  /*
  * 为执行监听器实例对象填充属性值
  * 根据类中的属性名称 获取 属性对应的setter
  * 如果没找到, 则直接通过反射方式获取field对象
  * 如果没找到, 根据 throwExceptionOnMissingField 参数值进行处理
  * 如果为true 程序报错, 否则直接返回
  *
  * 如果field不为空
  * 调用 fieldTypeCompatible() 设置属性值
  * */
  public static void applyFieldDeclaration(FieldDeclaration declaration, Object target, boolean throwExceptionOnMissingField) {
    Method setterMethod = ReflectUtil.getSetter(declaration.getName(),  //获取方法
      target.getClass(), declaration.getValue().getClass());
    
    if(setterMethod != null) {
      try {
        setterMethod.invoke(target, declaration.getValue());//调用setMethod() 进行属性赋值
      ...
      Field field = ReflectUtil.getField(declaration.getName(), target);//获取属性信息
      if(field == null) {
     ...
     ReflectUtil.setField(field, target, declaration.getValue());//设置属性值
