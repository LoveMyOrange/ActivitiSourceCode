...
public class DynamicBpmnServiceImpl extends ServiceImpl implements DynamicBpmnService, DynamicBpmnConstants {
  ...
  public ObjectNode getBpmnElementProperties(String id, ObjectNode infoNode) {
    ...
    if (bpmnNode != null) {
      //则将id 值 作为 查询条件 并且从bpmnNode 对象中 获取该 id 值下面所有的数据集合并且返回
  ...
  /*
   从infoNode中获取 key 为bpmn的值
   这个地方的处理有点问题,
   因为引擎并不会解析 activiti:bpmn元素
   所以获取该元素下 配置的节点缓存数据显然不合理, 不过没哟关系
   可能通过 手动构造数据并更新 缓存的方式使用该功能
   */
